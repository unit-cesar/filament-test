...
echo
read -p "Deseja efetuar os Testes Unitários (PHPUnit): npm run testDev (s ou n)? " continue_process
...
read -e -p "Informe a mensagem do commit: " commit_message
...
if [ "${create_tag,,}" = "s" ]; then
    read -e -p "Informe o nome da TAG: " tag_name
fi
...
echo
read -e -p "Qual o nome da nova Branch? " new_branch_name
# Remove caracteres não permitidos e formata o nome da branch
clean_branch_name=$(echo "$new_branch_name" | tr -d '[:space:]' | tr -c '[:alnum:]_-' '_')
git checkout -b "ts$(date +%s)-${clean_branch_name}"
